$(document).ready(function(){	
	...
	$("#employeeList").on('click', '.update', function(){
		...
		$.ajax({
			...
			success:function(data){
				$('#employeeModal').modal('show');
				$('#empId').val(data.id);
				$('#empProfessor').val(data.Professor);
				$('#empSala').val(data.Sala);
				$('#empDisciplina').val(data.Disciplina);				
				$('#Curso').val(data.Curso);
				$('#Dia').val(data.Dia);	
				$('#Obs').val(data.Obs);
				$('.modal-title').html("<i class='fa fa-plus'></i> Editar tabela");
	...
	$("#employeeList").on('click', '.delete', function(){
		...
		if(confirm("Você tem certeza em excluir essa reserva?")) {
			$.ajax({
				url:"action.php",
				method:"POST",
				data:{empId:empId, action:action},
